 ...
 * 
 */
bool DIRECTED_widestparh = false; // 边是否有向，默认为无向，即图为无向图
...
public:
  ...
  /**
   * \brief We use the messaging model to compute the widestparh update
   */
  ...
  /**
   * \brief Determine if widestparh should run on all edges or just in edges
   */
  edge_dir_type scatter_edges(icontext_type& context, 
                             const vertex_type& vertex) const {
    if(changed)
      return DIRECTED_widestparh? graphlab::OUT_EDGES : graphlab::ALL_EDGES; 
...
int main(int argc, char** argv) {
  ...
  clopts.attach_option("directed", DIRECTED_widestparh,
  ...
  // Running The Engine -------------------------------------------------------
  graphlab::omni_engine<widestparh> engine(dc, graph, exec_type, clopts);
...
// ./widestparh --graph graph.txt --source 1 --directed true --saveprefix widestparhRes --savefilenum 1